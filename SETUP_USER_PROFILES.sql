-- This script fixes the root cause of the "406 Not Acceptable" error by ensuring
-- that every user in the authentication system has a corresponding profile in the
-- public.users table. It also cleans up conflicting security policies.

-- Step 1: Clean up the old, conflicting RLS policies.
...
-- Step 2: Create a function to automatically create a user profile.
...
-- Step 3: Create a trigger that runs the function after a new user is created.
...
-- Step 4: Manually create a profile for the existing test user.
